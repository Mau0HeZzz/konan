...
import { isMobile, _slideUp, _slideDown, _slideToggle, FLS, getDigFromString } from "../functions.js";
...
export function formQuantity() {
  let disEvt = true;
	document.addEventListener("click", function (e) {
      ...
      if (parent) {
        ...
        if (input) {
          let value = getDigFromString(input.value);
          let max = input.max || Infinity;
          let min = input.min || 1;
          let step = getDigFromString(input.dataset.step) || 1;
          ...
          if (targetElement.classList.contains('quantity__button_plus')) {
            ...
            if (minusBtn.querySelector('i')) {
              minusBtn.querySelector('i').hidden = true;
            }
            if (value < max) {
              value+=step;
            } else {
            ...
            if (value <= min) {
              value = min;
              if (minusBtn.querySelector('i')) {
                minusBtn.querySelector('i').hidden = false;
              } else {
  ...
  document.addEventListener('change', (e) => {
    ...
    const parent = quantityInput.closest('[data-quantity]');
    let value = getDigFromString(quantityInput.value);
    let max = getDigFromString(quantityInput.max || Infinity);
    let min = getDigFromString(quantityInput.min || 1);
    let step = getDigFromString(quantityInput.dataset.step);
    if (isNaN(step) || step <= 0) step = 1;
    const plusBtn = parent.querySelector('.quantity__button_plus');
    const minusBtn = parent.querySelector('.quantity__button_minus');
    ...
    if (value <= min) {
      ...
      const ostatok = value % step;
      value = value + (step - ostatok);
    }
